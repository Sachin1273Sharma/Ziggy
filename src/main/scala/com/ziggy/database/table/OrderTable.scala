...
final class OrderTable@Inject()(db: Database)
	...
	def insertAll(values: Seq[Order]): Future[Option[Int]] = {
		db
			.run(orders ++=
			     values.map(order => order.copy(id = order.id.orElse(Some(UUID.randomUUID().toString))))
			     )
	}
	...
	def findByCustomerId(customerId: String): Future[Seq[Order]] = {
		db.run(orders.filter(_.customerId === customerId).sortBy(_.id.asc).result)
	}
		...
		val action = orders
			...
			.flatMap {
				case Some((order, customer, restaurant)) =>
					...
					} yield Some(OrderRoutingContext(
						order,
						customer,
						customerAddress,
						restaurant,
	...
	private def findCurrentCustomerAddress(customerId: Option[String]): DBIO[Option[Address]] = {
		customerId match {
			case Some(id) =>
				customerAddresses
	...
	def deleteByCustomerId(customerId: String): Future[Int] = {
		db.run(orders.filter(_.customerId === customerId).delete)
	}
