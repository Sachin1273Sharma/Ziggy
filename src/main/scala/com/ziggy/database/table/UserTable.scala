...
final class UserTable(db: Database)(implicit ec: ExecutionContext) {
  ...
  def insertAll(values: Seq[User]): Future[Option[Int]] =
    db.run(users ++= values.map(value => value.copy(id = value.id.orElse(Some(UUID.randomUUID())))))

  def findById(id: String): Future[Option[User]] =
    db.run(users.filter(_.id === UUID.fromString(id)).result.headOption)

	def findByRefId(refId : String) : Future[Option[User]] =
		db.run(users.filter(_.refId == refId).result.headOption)
  ...
  def update(id: String, customer: User): Future[Int] = {
    val updatedCustomer = customer.copy(id = Some(id))
    db.run(users.filter(_.id === id).update(updatedCustomer))
  }

	def updateAction(id: String, customer: User): DBIO[Int] = {
		val updatedCustomer = customer.copy(id = Some(id))
